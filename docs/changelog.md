...
- Close final release-review gaps by preserving authorized legacy context effects, materializing layout-specific context, deriving readiness from verified post-apply state, correcting setup recovery guidance, scoping Jira correlations by repository, and hardening synchronization with pre-write Local/Jira journals, recoverable Jira-pull patches, atomic bare-to-scoped pending-correlation migration, and unique crash-safe identities for repeated comments (WCM-1)
