...
- Add the canonical `.wsagency/config.yaml` policy, tracker provider adapters, hub-aware setup transaction, reusable documentation bootstrap, lossless pre-5 migration with read-back-gated cleanup, and safe resumable reconfiguration across Claude Code and omp (WCM-1)
