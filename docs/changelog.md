...
- Close final release-review gaps by preserving authorized legacy context effects, materializing layout-specific context, deriving readiness from verified post-apply state, correcting setup recovery guidance, scoping Jira correlations by repository, and hardening synchronization with pre-write Local/Jira journals, pre-inspected recoverable Jira-pull patches, unambiguous atomic bare-to-scoped pending-correlation migration, caller-stable crash-safe comment identities, and non-rendered Jira comment properties that leave authored comment bodies unchanged (WCM-1)
