...
- Make the herdr refresh contract actually carry the licence. Both the `ws-repo-maintenance` procedure and `plugins/ws/skills/herdr/UPSTREAM.md` now fetch `SKILL.md` **and** `LICENSE` from the same upstream ref, forbid mixing `master` with the pin, require a `NOTICE` check, and require verifying both vendored files before the pin is updated. Until this landed the contract was internally inconsistent and ungated: `UPSTREAM.md` claimed the licence travels with a refresh while its executable steps fetched and replaced `SKILL.md` only. That would not have deleted the vendored `LICENSE` — Gate 4 forbids blind recursive copies and deleting WS-only files — but nothing re-fetched or verified the licence against the new pin, so a refresh could leave a stale or mismatched licence beside updated skill content, and a newly added upstream `NOTICE` would go unnoticed
...
- Add the canonical `.wsagency/config.yaml` policy and sole `/ws-setup` entry point (ADR 0010), capability-specific Local/GitHub/GitLab/Jira setup, hub-aware transactions, reusable documentation bootstrap, lossless pre-5 migration with read-back-gated cleanup, and safe resumable reconfiguration across Claude Code and omp (WCM-1)
...
- Make the release gate reject dirty or unexpected commits, mismatched tarball digests, failed installation commands, unexpected health warnings, and stale Claude Code or omp plugin identities while accepting omp's exact clean-profile package-manifest initialization warning before publication (WCM-1)
- Close the final cutover review by containing every repository write, binding reconfiguration and Jira-backfill source freshness, preserving Claude settings and project-specific rule prose, classifying legacy scratch and rule remnants, and reporting cleanup recovery without rollback (WCM-1)
- Complete the release cutover by globally preflighting multi-repository setup, migrating each hub target from its own legacy state, requiring reviewed lossless adapter merges, preserving Local/Jira sync intent durably, binding resumable reconfiguration to per-repository snapshots, routing native ticket and ADR writes through canonical policy, and cryptographically binding generated omp surfaces to the merged marketplace commit (WCM-1)
- Close the publication gate with domain-scoped reconfiguration blocking, per-target hub backfill refresh, a production native Local/Jira synchronization boundary with durable real comment identities, and installed migration transactions for both distributions (WCM-1)
- Close final release-review gaps by preserving authorized legacy context effects, materializing layout-specific context, deriving readiness from verified post-apply state, correcting setup recovery guidance, scoping Jira correlations by repository, and hardening synchronization with pre-write Local/Jira journals, pre-inspected recoverable Jira-pull patches, queue-wide unambiguous pending-create invariants, atomic bare-to-scoped correlation migration, caller-stable crash-safe comment identities, and paginated non-rendered Jira comment properties that leave authored comment bodies byte-for-byte unchanged (WCM-1)
...
- End-to-end command, docs, hub, Matt-graph, hook, tool, and packaging review repaired the remaining flow defects: hub initialization/adoption is collision-safe and executable from every project shape; documentation sweeps and product-scope routing agree on hub/output boundaries; commit/PR flows handle empty commits and incremental worklogs; worker schemas and handoffs carry durable artifacts and blocked outcomes; changelog, OpenWiki, SessionStart, duplicate-install, ticket, ADR, and stop-nudge runtime paths now handle nested repos, untracked files, malformed input, named omp profiles, XDG layouts, and recovery without stale or misleading UI state
