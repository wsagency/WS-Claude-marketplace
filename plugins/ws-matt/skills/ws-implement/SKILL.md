...
Commit your work to the current branch. Commits follow the WS conventions — Conventional Commits with the ticket reference (`/ws-commit` when available). When the branch is complete, the PR flow is `/ws-commit-push-pr`, which also handles the CHANGELOG entry and the Jira transition when the project is bound — do NOT hand-write changelog entries in this skill; the PR-time entry is canonical.
...
- **Edges:**
  ...
  - then → /ws-commit-push-pr at branch completion (WS command: Conventional Commit + ticket reference, CHANGELOG entry, push, PR, Jira transition when bound)
