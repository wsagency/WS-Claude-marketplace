...
Send a single message with two `Agent` tool calls. Use the `ws-matt-reviewer` agent for both — one review assignment each (the Standards axis over the whole diff, and the Spec axis over the whole diff).
...
- **Edges:**
  - fan-out: for each axis spawn a ws-matt-reviewer agent in parallel — one Standards, one Spec (schema: findings per file/hunk, under 400 words, hard violations distinguished from judgement calls)
