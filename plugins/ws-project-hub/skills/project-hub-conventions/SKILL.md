...
4. **Marketplace check** — `git ls-remote $WS_MARKETPLACE_URL` (default `wsagency/ws-claude-marketplace`); compares against cached SHA in `~/.cache/ws-hub/known-marketplace-sha`. Prints first-time-add, update-available, or up-to-date message; the hint is agent-neutral (it names the marketplace add step for the chosen agent, not claude-only wording).
