...
- **Emits:** throwaway prototype code (interactive terminal app per [LOGIC.md](LOGIC.md), or structurally different UI variants per [UI.md](UI.md)) with full state surfaced; the validated answer; the prototype captured on a throwaway branch with a context pointer on the implementation issue
- **Edges:**
...
- **Exit report:** nested under a driver, return the verdict and the throwaway-branch pointer as state delta and emit no route; invoked directly, report the verdict — validated or rejected — and stop, no invented driver. (Format: `ws-graph-engineering`.)
