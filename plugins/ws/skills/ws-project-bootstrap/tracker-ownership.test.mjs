...
test("every existing store requires one explicit supported disposition and cancel leaves ownership unchanged", () => {
	...
	assert.throws(() => planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: { ...choices(), sourceStores: ["OLD", "SECOND"], dispositions: [{ storeId: "OLD", disposition: "copy-all" }] } }), error => error.code === "ERR_MISSING_DISPOSITIONS");
	assert.throws(() => planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: {
		...choices(),
		dispositions: [
...
test("preserve-as-history, copy-selected, copy-open, and copy-all dispositions are deterministic", () => {
	...
	const preserved = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices("preserve-as-history", {
	...
	const selected = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices("copy-selected", {
	...
	const open = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices("copy-open") });
	...
	const all = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices("copy-all") });
...
test("copies preserve source links, list semantic loss, and never plan source mutation", () => {
	...
	const planned = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices() });
...
test("claimed work, unresolved conflicts, and pending sync block only affected copies unless explicitly excluded", () => {
	...
	const blocked = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: sync, choices: choices() });
	...
	const excluded = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: sync, choices: choices("copy-all", { excludedItemIds: ["OLD-1", "OLD-2", "OLD-3"] }) });
...
test("Jira project rebinding preserves old keys as inactive history and creates verified copies instead of moving", () => {
	...
	const planned = planTrackerOwnership({ config: JIRA_CONFIG, localStore: store, syncState: {}, choices: choices("copy-all", {
		targetTracker: "jira",
		sourceProject: "OLD",
		targetProject: "NEW",
		sourceStores: ["OLD"],
...
test("external creates use stable correlations and journal each returned identity before ownership cutover", async () => {
	...
	const planned = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices() });
	const repeated = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices() });
	...
	await applyTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices() }, { planHash: planned.hash, effects: planned.effects, adapters: adapters, injection: { failAtPhase: "cutover" } });
...
test("remote identity/version/mapped-field drift is re-fetched immediately before mutation and stops writes", async () => {
	const store = { "OLD-1": ticket("OLD-1") };
	const planned = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices("copy-all", { remoteFingerprints: { "OLD-1": { identity: "candidate", version: 1, mappedFieldHash: "v1" } } }) });
	let refetches = 0;
	const adapters = mockAdapters({
	...
	const result = await applyTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices("copy-all", { remoteFingerprints: { "OLD-1": { identity: "candidate", version: 1, mappedFieldHash: "v1" } } }) }, { planHash: planned.hash, effects: planned.effects, adapters: adapters });
...
test("interruption resumes without duplicate remote creates and verifies final ownership before audit cleanup", async () => {
	...
	const planned = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: selectedChoices });
	const adapters = mockAdapters();
	const interrupted = await applyTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: selectedChoices }, { planHash: planned.hash, effects: planned.effects, adapters: adapters, injection: { failAtPhase: "cutover" } });
	assert.equal(interrupted.success, false);
	const createsBefore = adapters.getApplied().filter(id => id.startsWith("prepare:remote:"));
	const resumed = await resumeTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: selectedChoices }, { adapters: adapters });
...
test("canonical ownership verification failure keeps the journal and prevents cleanup completion", async () => {
	...
	const result = await applyTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: choices() }, { planHash: planned.hash, effects: planned.effects, adapters: adapters });
	assert.equal(result.success, false);
...
test("aligned preserve-as-history ownership is a no-op with no journal", async () => {
	...
	const planned = planTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: selectedChoices });
	const adapters = mockAdapters();
	const result = await applyTrackerOwnership({ config: BASE_CONFIG, localStore: store, syncState: {}, choices: selectedChoices }, { planHash: planned.hash, effects: planned.effects, adapters: adapters });
