...
- **Reads:** the spec or ticket being built (tracker issue or `dev-docs/tickets/open/` file), the pre-agreed test seams, `CONTEXT.md`, ADRs, and `dev-docs/agents/issue-tracker.md` for the ticket close-out workflow
...
- **Handoff protocol:** the work lands as commits on the current branch; close out on the driving ticket per `dev-docs/agents/issue-tracker.md` — for the local tracker, move `dev-docs/tickets/open/<slug>.md` to `done/` once the result is coded and anything worth keeping has landed in dev-docs — and reference it by id (DONE|{branch@commit}).
