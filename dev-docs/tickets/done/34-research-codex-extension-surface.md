...
Distribution has a direct path for us: plugin marketplaces read `.agents/plugins/marketplace.json` **and the legacy `.claude-plugin/marketplace.json`**, with `codex plugin marketplace add` accepting GitHub, Git, local, and npm sources (npm installs run no lifecycle scripts). A reviewed universal Plugins Directory exists for public listing (verified developer identity, five positive and three negative test cases, HTTPS-only remote MCP, tool annotations), and enterprise GitHub workspace import is available.
