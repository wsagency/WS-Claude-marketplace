...
- ws-matt graph contracts now route product ADRs to the parent hub's `dev-docs/decisions/` while keeping repo- and bounded-context ADRs local, name the `researcher` worker explicitly, and support optional repeated waves of independent `tdd-runner` red-green cycles from `ws-implement`; source, reference, and generated omp surfaces are aligned
